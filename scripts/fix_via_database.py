"""
Update query_context directly in Superset metadata database.
"""
...
def psql(sql):
    """Run SQL against Superset metadata database."""
    cmd = f'docker exec academic-datalakehouse-postgres-1 psql -U academic -d superset -c "{sql}"'
...
def main():
    ...
    print("=" * 70)
    print("UPDATE QUERY_CONTEXT VIA DATABASE")
    print("=" * 70)
    ...
    # Update each chart's query_context directly in the database
    for c in charts:
        cid = c["id"]
        name = c["slice_name"]
        ...
        qc_json = json.dumps(qc).replace("'", "''")
        
        sql = f"UPDATE slices SET query_context = '{qc_json}' WHERE id = {cid}"
        result = psql(sql)
        ...
        if "UPDATE 1" in result:
            ...
            print(f"  Chart {cid:2d}: {name:45s} -> FAILED")
        ...
        if r2.status_code == 200:
            try:
                ...
                if "result" in data and data["result"]:
                    ...
                    print(f"  Chart {cid:2d}: OK ({rc} rows)")
                    ok += 1
                else:
                    print(f"  Chart {cid:2d}: OK (empty)")
                    ok += 1
            except:
                print(f"  Chart {cid:2d}: ERROR")
                fail += 1
        else:
            try:
                ...
                err = r2.text[:60]
            print(f"  Chart {cid:2d}: FAIL ({r2.status_code}): {err}")
            fail += 1
