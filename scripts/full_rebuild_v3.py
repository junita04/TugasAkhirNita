"""
DASHBOARD REBUILD: 24 charts, 6 sections
ML metrics at BOTTOM in one row (3+3+3+3=12)
"""
...
charts = {
    ...
    81: "Akurasi", 82: "Presisi", 83: "Recall", 84: "F1-Score",
    85: "Confusion Matrix",
...
# Rename ML metric titles
for cid, title in charts.items():
    if cid == 81: charts[cid] = "Akurasi"
    elif cid == 82: charts[cid] = "Presisi"
    elif cid == 83: charts[cid] = "Recall"
    elif cid == 84: charts[cid] = "F1-Score"

# Update chart names in Superset
for cid in [81, 82, 83, 84]:
    r = s.put(f"{BASE}/api/v1/chart/{cid}", json={"slice_name": charts[cid]})
...
# Section 2: Profil Mahasiswa (manual for custom widths)
...
# Section 6: Hasil Evaluasi Model Machine Learning (4 metrics in one row, 3+3+3+3=12)
add_section(6, "Hasil Evaluasi Model Machine Learning", [
    [81, 82, 83, 84],
])

# Fix KPI widths (3+3+2+2+2=12)
